/**
 ******************************************************************************
 *
 * @file aes256_enc.c
 *
 * This code is part of the String Obfuscation Project by Michael Haephrati 
 * The source code is licensed under COMMON DEVELOPMENT AND DISTRIBUTION LICENSE (CDDL)Version 1.1
 * © 2012 Michael Haephrati - haephrati@gmail.com
 *
 * @brief @~english Implementatoion of function for encode string.
 *
 *
 *
 * <b>Modification History</b>
 *
 * extern "C" wchar_t* __ODC__( const char * );
 ******************************************************************************
 * @n @b encoding:    US-ASCII
 * @n @b tabsize:     8 (not used)
 * @n @b indentation: 2
 */

#include <stdlib.h>
#include <stdio.h>
#include <io.h>
#include <fcntl.h>
#include <time.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <memory.h>
#include <wchar.h>
#include <windows.h>

#ifndef BYTE
#define BYTE  unsigned char
#endif

/* -------------------------------------------------------------------------- */

typedef struct 
{
  BYTE key[32]; 
  BYTE enckey[32]; 
  BYTE deckey[32];
} aes_key; 

/* -------------------------------------------------------------------------- */

static unsigned r_filename = 0;
static unsigned r_filesize = 0;
static unsigned r_filedate = 0;


/* -------------------------------------------------------------------------- */
const BYTE sbox_enc[256] = {
    
};
#define rj_sbox_enc(x)  sbox_enc[(x)]

/* -------------------------------------------------------------------------- */
BYTE rj_xtime(BYTE x) 
{
    return (x & 0x80) ? ((x << 1) ^ 0x1b) : (x << 1);
}

/* -------------------------------------------------------------------------- */
void aes_subBytes_enc( BYTE *buf )
{
    register BYTE i = 16;

    while (i--) buf[i] = rj_sbox_enc(buf[i]);
}

/* -------------------------------------------------------------------------- */
void aes_addRoundKey(BYTE *buf, BYTE *key)
{
    register BYTE i = 16;

    while (i--) buf[i] ^= key[i];
}

/* -------------------------------------------------------------------------- */
void aes_addRoundKey_cpy( BYTE *buf, BYTE *key, BYTE *cpk )
{
    register BYTE i = 16;

    while (i--)  buf[i] ^= (cpk[i] = key[i]), cpk[16+i] = key[16 + i];
}


/* -------------------------------------------------------------------------- */
void aes_shiftRows_enc(BYTE *buf)
{
    register BYTE i, j; /* to make it potentially parallelable :) */

    i = buf[1]; buf[1] = buf[5]; buf[5] = buf[9]; buf[9] = buf[13]; buf[13] = i;
    i = buf[10]; buf[10] = buf[2]; buf[2] = i;
    j = buf[3]; buf[3] = buf[15]; buf[15] = buf[11]; buf[11] = buf[7]; buf[7] = j;
    j = buf[14]; buf[14] = buf[6]; buf[6]  = j;

}

/* -------------------------------------------------------------------------- */
void aes_mixColumns(BYTE *buf)
{
    register BYTE i, a, b, c, d, e;

    for (i = 0; i < 16; i += 4)
    {
        a = buf[i]; b = buf[i + 1]; c = buf[i + 2]; d = buf[i + 3];
        e = a ^ b ^ c ^ d;
        buf[i] ^= e ^ rj_xtime(a^b);   buf[i+1] ^= e ^ rj_xtime(b^c);
        buf[i+2] ^= e ^ rj_xtime(c^d); buf[i+3] ^= e ^ rj_xtime(d^a);
    }
}

/* -------------------------------------------------------------------------- */
void aes_mixColumns_inv(BYTE *buf)
{
    register BYTE i, a, b, c, d, e, x, y, z;

    for (i = 0; i < 16; i += 4)
    {
        a = buf[i]; b = buf[i + 1]; c = buf[i + 2]; d = buf[i + 3];
        e = a ^ b ^ c ^ d;
        z = rj_xtime(e);
        x = e ^ rj_xtime(rj_xtime(z^a^c));  y = e ^ rj_xtime(rj_xtime(z^b^d));
        buf[i] ^= x ^ rj_xtime(a^b);   buf[i+1] ^= y ^ rj_xtime(b^c);
        buf[i+2] ^= x ^ rj_xtime(c^d); buf[i+3] ^= y ^ rj_xtime(d^a);
    }
}

/* -------------------------------------------------------------------------- */
void aes_expandEncKey(BYTE *k, BYTE *rc) 
{
    register BYTE i;

    k[0] ^= rj_sbox_enc(k[29]) ^ (*rc);
    k[1] ^= rj_sbox_enc(k[30]);
    k[2] ^= rj_sbox_enc(k[31]);
    k[3] ^= rj_sbox_enc(k[28]);
    *rc = (((*rc)<<1) ^ ((((*rc)>>7) & 1) * 0x1b));

    for(i = 4; i < 16; i += 4)  k[i] ^= k[i-4],   k[i+1] ^= k[i-3],
        k[i+2] ^= k[i-2], k[i+3] ^= k[i-1];
    k[16] ^= rj_sbox_enc(k[12]);
    k[17] ^= rj_sbox_enc(k[13]);
    k[18] ^= rj_sbox_enc(k[14]);
    k[19] ^= rj_sbox_enc(k[15]);

    for(i = 20; i < 32; i += 4) k[i] ^= k[i-4],   k[i+1] ^= k[i-3],
        k[i+2] ^= k[i-2], k[i+3] ^= k[i-1];

}

/* -------------------------------------------------------------------------- */
void aes_prepare( aes_key *key_context, BYTE *k )
{
    BYTE rcon = 1;
    register BYTE i;

    for (i = 0; i < sizeof(key_context->key); i++) key_context->enckey[i] = key_context->deckey[i] = k[i];
    for (i = 8;--i;) aes_expandEncKey(key_context->deckey, &rcon);
}

/* -------------------------------------------------------------------------- */
void aes_encrypt_block( aes_key *key_context, BYTE *buf )
{
    BYTE i, rcon;

    aes_addRoundKey_cpy( buf, key_context->enckey, key_context->key );
    for(i = 1, rcon = 1; i < 14; ++i)
    {
        aes_subBytes_enc(buf);
        aes_shiftRows_enc(buf);
        aes_mixColumns(buf);
        if( i & 1 ) aes_addRoundKey( buf, &key_context->key[16]);
        else aes_expandEncKey(key_context->key, &rcon), aes_addRoundKey(buf, key_context->key);
    }
    aes_subBytes_enc(buf);
    aes_shiftRows_enc(buf);
    aes_expandEncKey(key_context->key, &rcon); 
    aes_addRoundKey(buf, key_context->key);
}


/* -------------------------------------------------------------------------- */
int AES_encode_w( unsigned int key_start, const wchar_t * plainString, unsigned char * outbuf, unsigned outlen )
{
  unsigned char key[32];
  aes_key key_context = {0};
  int i;
  unsigned char offset;

  /** Calculate required size */
  int retval = (wcslen(plainString) + 1)*sizeof(wchar_t);
  /** Round to 16 byte over */
  retval = ((retval + 15)&(~0xF)) + 4;

  /** Memory request */
  if( NULL == outbuf )
    return -retval;

  /** Not enought memory */
  if( outlen < retval )
    return 0;

  /** Prepare output buffer */
  memset( outbuf, 0, retval );
  wcscpy( (wchar_t*)(outbuf+4), plainString );
  *((unsigned*)outbuf) = key_start;
  

  /** Prepare key */
  srand(key_start);

  for( i = 0; i < sizeof(key); i++ )
    key[i] = rand();

  aes_prepare( &key_context, key );
  memset( key, 0, sizeof(key) );

  for( i = 4; i < retval; i += 16 )
  {
    aes_encrypt_block( &key_context, &outbuf[i] );
  }

  memset( &key_context, 0, sizeof(key_context) );

  return retval;
}
/* -------------------------------------------------------------------------- */
int AES_encode_a( unsigned int key_start, const wchar_t * plainString, unsigned char * outbuf, unsigned outlen )
{
  unsigned char key[32];
  aes_key key_context = {0};
  int i;
  unsigned char offset;

  /** Calculate required size */
  int retval = (wcslen(plainString) + 1);
  /** Round to 16 byte over */
  retval = ((retval + 15)&(~0xF)) + 4;

  /** Memory request */
  if( NULL == outbuf )
    return -retval;

  /** Not enought memory */
  if( outlen < retval )
    return 0;

  /** Prepare output buffer */
  memset( outbuf, 0, retval );
//  wcscpy( (char*)(outbuf+4), plainString );
  WideCharToMultiByte( CP_ACP, 0, plainString, -1, (outbuf+4), retval-sizeof(unsigned),NULL, NULL);

  *((unsigned*)outbuf) = key_start;
  

  /** Prepare key */
  srand(key_start);

  for( i = 0; i < sizeof(key); i++ )
    key[i] = rand();

  aes_prepare( &key_context, key );
  memset( key, 0, sizeof(key) );

  for( i = 4; i < retval; i += 16 )
  {
    aes_encrypt_block( &key_context, &outbuf[i] );
  }

  memset( &key_context, 0, sizeof(key_context) );

  return retval;
}
/* -------------------------------------------------------------------------- */
