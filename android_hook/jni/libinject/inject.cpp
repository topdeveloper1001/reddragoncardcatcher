...
static pid_t _pid;
static void *_dlopen;
static void *_dlerror;
static void *_calloc;
static void *_free;

typedef void (*remote_stop_t)();
static remote_stop_t remote_stop_ptr = NULL;
...
#endif
            default:
                snprintf(rb, sizeof(rb), "%d", request);
                r = rb;
            }
...
static uintptr_t remote_call(void *function, int nargs, ...) {
    ...
    // push parameters into registers and stacks, setup registers to perform the
    // call
...
static void remote_free(uintptr_t p) { remote_call(_free, 1, p); }
...
int libinject_inject(pid_t pid, const char *library) {
    ...
    if (trace("attach", PTRACE_ATTACH) != -1) {
        ...
        /* We can resolve the references to LIBC easily, but dl* is tricky. On
         * older Android versions, libdl.so is commonly not loaded by the
         * linker, and our dl* functions come directly from the linker. On newer
         * Android versions, libdl.so is directly loaded and dl* come from
         * there.
         */
        _calloc = remote_findFunction(PATH_LIBC, reinterpret_cast<void *>(calloc));
        _free = remote_findFunction(PATH_LIBC, reinterpret_cast<void *>(free));
        if ((findLibrary(PATH_LIBDL, -1) != 0) && (findLibrary(PATH_LIBDL, _pid) != 0)) {
            ...
            _dlopen = remote_findFunction(PATH_LINKER, reinterpret_cast<void *>(dlopen));
            _dlerror = remote_findFunction(PATH_LINKER, reinterpret_cast<void *>(dlerror));
        }

        INJECTLOG("calloc:%p free:%p dlopen:%p dlerror:%p", static_cast<void *>(_calloc),
                  static_cast<void *>(_free), static_cast<void *>(_dlopen),
                  static_cast<void *>(_dlerror));
